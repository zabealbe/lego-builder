...
#class that define the building
@dataclass
#########################
#class that defines a block in the building:
...
class block:
    ...
    P: float
    Y: float
    def __init__ (self,lego_name,x,y,z,R,P,Y):
        ...
        self.Y=Y
#########################
#class that defines a layer of the building
#########################
class layer:
    def __init__ (self):
        self.legos=[]
...
#############################
#parser function: used to parse the structure of the building from the json file building.json into a layer structure
#############################

def parser():
    f= open('building2.json')
    l = json.load(f)
    layer_num = l['layer_number']
    Layers = []
    for i in range(0,int(layer_num)):
        ll = layer()
        Layers.append(ll)
    layerx = 'layer'
    for L in range(0,int(layer_num)):
        layer_name = layerx+str(L+1)
        blocks_number=int(l[layer_name][0]['blocks'])
        for b in range(1,blocks_number+1):
            name=l[layer_name][b]['name']
            x=l[layer_name][b]['x']
            y=l[layer_name][b]['y']
            z=l[layer_name][b]['z']
            R=l[layer_name][b]['R']
            P=l[layer_name][b]['P']
            Y=l[layer_name][b]['Y'] 
            lego = block(name,x,y,z,R,P,Y) 
            Layers[L].legos.append(lego)
...
#############################
def move_to_destination(model_pose, model_name, model_index,x_dest,y_dest,z_dest,R_dest,P_dest,Y_dest):
    close_gripper(model_name)
    z_dest-=0.02
    X=model_pose.position.x
    Y=model_pose.position.y
    ...
    qx = np.sin(R_dest/2) * np.cos(P_dest/2) * np.cos(Y_dest/2) - np.cos(R_dest/2) * np.sin(P_dest/2) * np.sin(Y_dest/2)
    qy = np.cos(R_dest/2) * np.sin(P_dest/2) * np.cos(Y_dest/2) + np.sin(R_dest/2) * np.cos(P_dest/2) * np.sin(Y_dest/2)
    qz = np.cos(R_dest/2) * np.cos(P_dest/2) * np.sin(Y_dest/2) - np.sin(R_dest/2) * np.sin(P_dest/2) * np.cos(Y_dest/2)
    qw = np.cos(R_dest/2) * np.cos(P_dest/2) * np.cos(Y_dest/2) + np.sin(R_dest/2) * np.sin(P_dest/2) * np.sin(Y_dest/2)
    
    final_quat=PyQuaternion(qw,qx,qy,qz)
    angle = get_approach_angle(final_quat,"up")
    final_quat=get_approach_quat("up",angle)

    move_smooth(initial_point,final_point,DEFAULT_QUAT,final_quat,mode=0)

    #move_smooth(initial_point,final_point,DEFAULT_QUAT,final_quat,mode=0)
    Send_joints(x_dest,y_dest,z_dest+SAFE_Z_OFFSET, final_quat, 10)
    check_position_loop() 
    #Send_joints(x_dest,y_dest,z_dest+0.025, final_quat, 10) 
    initial_point=final_point
    final_point=[x_dest,y_dest,z_dest+0.025]
    move_smooth(initial_point,final_point,final_quat,final_quat,mode=0)
    '''
    initial_point==final_point
...
if __name__ == "__main__":
    ...
    #node init for joints
    rospy.init_node("send_joints")
    #initialize publisher for trajectory controller messages
    pub = rospy.Publisher("/trajectory_controller/command",JointTrajectory,queue_size=10)
    #initialization of services
    setstatic_srv = rospy.ServiceProxy('/link_attacher_node/setstatic', SetStatic) #set static service
    attach_srv = rospy.ServiceProxy('/link_attacher_node/attach', Attach) #attach service
    detach_srv = rospy.ServiceProxy('/link_attacher_node/detach', Attach) #detach service
    setstatic_srv.wait_for_service()
    attach_srv.wait_for_service()
    detach_srv.wait_for_service()
    
    #initialization of the trajectory controller
    traj = JointTrajectory()
    traj.header = Header()
    #defining names of the joints that will be controlled by trajectory controller
    traj.joint_names = ['shoulder_pan_joint', 'shoulder_lift_joint','elbow_joint', 'wrist_1_joint', 'wrist_2_joint','wrist_3_joint']
    #name=sys.argv[1]
    #creates the building structure parsing from json file
    Layers = parser()
    #gets Gazebo lego models positions
    legos = get_legos_pos()
    #print(legos)
    ...
    for i in Layers:
        for j in i.legos:
            ...
            for model_name, model_pose in legos:
                ...
                print(model_name,j.lego_name)
                if fixed_model_name not in j.lego_name:        
                    continue
                try:
                ...
                move_to_destination(model_pose, model_name, model_index,j.x,j.y,j.z,j.R,j.P,j.Y)
